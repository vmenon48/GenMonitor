...
queryMed.set("_sort", "-date");
  ...
  client.request("Observation?" + query, {
    ...
    function(ob) {
	  ...
	  if(wt1[0] != "undefined")
		weightId.innerHTML= wt1[0].id;

      displayObservation(p)

    });
	
client.request("MedicationRequest?" + queryMed, {
  pageLimit: 0,
  flat: true
}).then(
  function(ob) {
	  ...
	  for (var j = 0; j < ob.length; j++){
			...
			console.log(ob[j].medicationCodeableConcept.text);
			displayMedication(ob[j].medicationCodeableConcept.text);
...
//var now = new Date.prototype.format("dd/MM/yyyy hh:mm TT");
...
//var queryWt = new URLSearchParams();
//var patientId = client.patient.id;

  //queryWt.set("patient", client.patient.id);
  //queryWt.set("_count", 1);
  //queryWt.set("_sort", "-date");
  //queryWt.set("code", [
   // 'http://loinc.org|29463-7' ,//weight
//].join(","));

var weightIdAnnotation = document.getElementById('weightId').value; 
  ...
  //client.update
  displayAnnotation("Note: "+annotation+" added by Vidya on "+ currentdate);
